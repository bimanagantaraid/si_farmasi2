    ...
    class Data extends CI_Controller{
        ...
        function getObatMasukGudang(){
            ...
            foreach ($list as $item) {
                ...
                $row[] = $item->BulanMasuk;
                $row[] = $item->TahunMasuk;
                $row[] = "<a href='" . base_url('obatmasuk/data/update/') . $item->IdObatMasuk . "'><button type='button' class='btn btn-sm btn-primary m-1'>Edit</button></a><a href='" . base_url('obatmasuk/data/deleteOMG/') . $item->IdObatMasuk . "'><button type='button' class='btn btn-sm btn-danger m-1'>Hapus</button></a>";
                $data[] = $row;
            }
            $output = array(
                "draw" => @$_POST['draw'],
        ...
        function generateObatMasukGudang(){
            ...
            if($resultGenerate === true){
                $this->session->set_flashdata('success', 'Generate data Berhasil');
            }else{
                $this->session->set_flashdata('failed', 'Generate data Gagal');
            }
        }
        ...
        function insertOMG(){
            ...
            if($resultInsert === 'duplicated'){
                $this->session->set_flashdata('duplicated','data sudah tersedia, silahkan cari dan edit');
            }else if($resultInsert === true){
                $this->session->set_flashdata('success','Tambah data Berhasil');
            }else{
                $this->session->set_flashdata('failed', 'Tambah data Gagal');
            }
            redirect(base_url('obatmasuk/data'),'refresh');
        }
        ...
        function deleteOMG($IdObatMasuk){
            ...
            redirect(base_url('obatmasuk/data'));
        }
